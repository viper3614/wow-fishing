 ...
 */
@Slf4j
public class AutoFish implements AutoCloseable {
     ...
     */
    public static int findFloatCount = 6;
    /**
     * 总甩竿次数
     */
    public static int loopCount = 100;
    public static int fishCount = 0;
    public static int notfoundFloatCount = 0;

    /**
     * 10分钟Millis
     */
    public static long tenMinutes = 600000l;
    public static long oneHour = 60 * 60 * 1000l;
    /**
     * 设置程序执行时长，单位：小时   
     */
    public static long executeHours = 8;
    /**
     * 鱼漂图样
     */
    ...
    static {
        ...
        cutImtemplateImg = Imgcodecs.imread("D:\\Develop-Application\\Idea-projects\\wow-fishing\\src\\main\\resources\\1.png");
    ...
    public static void init(Robot robot) throws Exception {
        ...
        executor.submit(new FishTask(filePath));
    }
     ...
     * 通过毫秒数计算出时分秒
     */
    public static String spendTime(long millis) {
        //通过毫秒数计算出时分秒
    ...
    public static void calcTime(Robot robot) {
        executor.submit(() -> {
            ...
            long spend = 0;
            if (start == frequency) {
                spend = currented - start;
            } else {
                spend = currented - frequency;
            }
//            if (spend > tenMinutes) {
            if (spend > oneHour) {
                try {
    ...
    private static boolean goToFloat(Robot robot, int loopCount, int count) throws Exception {
        ...
        if (floatCoordinate == null) {
            ...
            Thread.sleep(100);
            log.info("没有找到鱼漂位置,开始第{}次重新定位鼠标坐标...", (findFloatCount++));
    ...
    public static Point detectObject(Mat sourceImage, Mat templateImage, FishType type) {
        ...
        log.info("detectObject() 图片识别 type:{}, 识别度minVal:{}", type.getName(), minVal);
        if (minVal < 0.35) {
    ...
    @Override
    public void close() {
        log.info("本次执行{}次,没找到鱼漂次数:{},是否关机:{},用时:{}", fishCount, notfoundFloatCount, poweroff, spendTime(System.currentTimeMillis() - start));
        if (poweroff) {
            try {
